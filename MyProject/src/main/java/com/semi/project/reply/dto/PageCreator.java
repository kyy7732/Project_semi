...
public class PageCreator {
	...
	private void calcDataOfPage() {
		...
		// 끝 페이지 번호 구하기 (올림( 현재 위치한 페이지 / 한 화면에 보여질 버튼 수) * 한 화면에 보여질 버튼 수
		this.end = (int) (Math.ceil((double) page.getPageNo() / BUTTON_NUM) * BUTTON_NUM);
		
		// 시작 페이지 구하기 ( 끝 페이지 번호 - 한 화면에 보여질 버튼 수 + 1 )
		this.begin = end - BUTTON_NUM + 1; 
		
		// 이전 버튼 활성화 = this.prev = (begin == 1) ? false : true;
		this.prev = begin > 1;
